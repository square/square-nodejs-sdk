...
describe('OrdersApi Tests', () => {
  ...
  it('should  testCreateOrder response', async () => {
    ...
    let { result, statusCode } = await ordersApi.createOrder(locationId, body)
  ...
  it('should testBatchRetrieveOrders response', async () => {
    ...
    let { result, statusCode } = await ordersApi.batchRetrieveOrders(locationId, {
      orderIds: [newOrderId],
    })
  ...
  it('should testUpdateOrder response', async () => {
    ...
    let { result, statusCode } = await ordersApi.updateOrder(locationId, newOrderId, body);
    expect(statusCode).toBe(200);
    expect(result.order).toEqual(expect.objectContaining({
      id: newOrderId,
