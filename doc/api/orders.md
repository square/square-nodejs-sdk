...
try {
  const { result, ...httpResponse } = await ordersApi.createOrder(locationId, body);
...
try {
  const { result, ...httpResponse } = await ordersApi.batchRetrieveOrders(locationId, body);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch(error) {
  if (error instanceof ApiError) {
...
try {
  const { result, ...httpResponse } = await ordersApi.updateOrder(locationId, orderId, body);
  // Get more response info...
  // const { statusCode, headers } = httpResponse;
} catch(error) {
  if (error instanceof ApiError) {
