...
export class Payouts {
    ...
    public async list(
        ...
        const list = core.HttpResponsePromise.interceptFunction(
            async (request: Square.ListPayoutsRequest): Promise<core.WithRawResponse<Square.ListPayoutsResponse>> => {
                ...
                if (_response.ok) {
                    return { data: _response.body as Square.ListPayoutsResponse, rawResponse: _response.rawResponse };
                }
    ...
    private async __get(
        ...
        const { payout_id: payoutId } = request;
        const _response = await (this._options.fetcher ?? core.fetcher)({
            url: core.url.join(
                (await core.Supplier.get(this._options.baseUrl)) ??
                    (await core.Supplier.get(this._options.environment)) ??
        ...
        if (_response.ok) {
            return { data: _response.body as Square.GetPayoutResponse, rawResponse: _response.rawResponse };
        }
    ...
    public async listEntries(
        ...
        const list = core.HttpResponsePromise.interceptFunction(
            async (
                ...
                const { payout_id: payoutId, sort_order: sortOrder, cursor, limit } = request;
                const _queryParams: Record<string, string | string[] | object | object[] | null> = {};
                if (sortOrder !== undefined) {
                    _queryParams["sort_order"] = sortOrder;
                }
                ...
                if (_response.ok) {
                    return {
                        data: _response.body as Square.ListPayoutEntriesResponse,
                        rawResponse: _response.rawResponse,
                    };
                }
                if (_response.error.reason === "status-code") {
        ...
        return new core.Pageable<Square.ListPayoutEntriesResponse, Square.PayoutEntry>({
            ...
            hasNextPage: (response) =>
                response?.cursor != null && !(typeof response?.cursor === "string" && response?.cursor === ""),
            getItems: (response) => response?.payout_entries ?? [],
            loadPage: (response) => {
                return list(core.setObjectProperty(request, "cursor", response?.cursor));
            },
        });
